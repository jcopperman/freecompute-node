...
// Public system status endpoint - basic info without authentication
app.get('/api/system/status', async (req, res) => {
  const registry = await loadRegistry();
  
  // Return basic system status for dashboard
  res.json({
    node: {
      name: registry.node.name,
      role: registry.node.role,
      uptime: registry.node.uptime || new Date().toISOString(),
      lastUpdated: new Date().toISOString()
    },
...
app.get('/api/system/status/detailed', authenticate, async (req, res) => {
